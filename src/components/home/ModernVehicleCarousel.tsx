...
export function ModernVehicleCarousel({
  ...
  const slidesToShow = Math.min(3, displayVehicles.length || 3)
  const align = slidesToShow >= 3 ? 'start' : 'center'
  const loop = displayVehicles.length > slidesToShow
  ...
  return (
    <>
      <div className="relative px-2 sm:px-4 lg:px-6">
        ...
        >
          <CarouselContent className="py-6 -mr-2 sm:-mr-4">
            {displayVehicles.map((vehicle) => (
              <CarouselItem
                key={vehicle.id}
                className="pr-2 sm:pr-4 basis-full sm:basis-3/4 md:basis-1/2 lg:basis-1/3"
              >
          ...
          {displayVehicles.length > slidesToShow && (
            <>
