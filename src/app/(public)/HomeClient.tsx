...
export default function HomeClient({ initialData = {} }: HomeClientProps) {
  ...
  // Initialize state with server props or empty defaults
  const [featuredVehicles, setFeaturedVehicles] = useState<PublicVehicle[]>(initialData.vehicles || [])
  const [totalVehiclesCount, setTotalVehiclesCount] = useState<number | null>(initialData.totalVehicles || null)
  const [sliderItems, setSliderItems] = useState<SliderItem[]>(initialData.sliders || [])
  const [companyInfo, setCompanyInfo] = useState<any>(initialData.companyInfo || null)
  const [siteSettings, setSiteSettings] = useState<any>(initialData.siteSettings || null)
  const [serviceItems, setServiceItems] = useState<any[]>(initialData.serviceItems || [])
  const [companyStats, setCompanyStats] = useState<any[]>(initialData.companyStats || [])
  const [companyValues, setCompanyValues] = useState<any[]>(initialData.companyValues || [])
  const [companyFeatures, setCompanyFeatures] = useState<any[]>(initialData.companyFeatures || [])
  const [timelineEvents, setTimelineEvents] = useState<any[]>(initialData.timelineEvents || [])
  const [contactInfo, setContactInfo] = useState<any>(initialData.contactInfo || null)

  // Initialize settings with server data or defaults
  const [homepageSettings, setHomepageSettings] = useState(initialData.homepageSettings || {
    showHeroSlider: true,
    autoPlaySlider: true,
    sliderInterval: 5000,
    showServices: true,
  ...
  // Loading state is true only if we don't have critical data
  const [loading, setLoading] = useState(!initialData.vehicles && !initialData.companyInfo)
  const [sliderLoading, setSliderLoading] = useState(!initialData.sliders)
  const [error, setError] = useState<string | null>(null)
  const [currentSlide, setCurrentSlide] = useState(0)
  const [isAutoPlay, setIsAutoPlay] = useState(initialData.homepageSettings?.autoPlaySlider ?? true)
  const [sliderInterval, setSliderInterval] = useState(initialData.homepageSettings?.sliderInterval ?? 5000)
